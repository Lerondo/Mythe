using UnityEngine;
using System.Collections;

[System.Serializable]
public class KneeCaps : Item {
	public KneeCaps()
	{
		itemSort = ItemSort.Legs;
		itemQuality = ItemQuality.Uncommon;
		_name = "Simple KneeCaps";
		_damage = 0;
		_magicDamage = 0;
		_defence = 4;
		_levelRequirement = 3;
		_buyValue = (_damage + _defence) * 3;
		_sellValue = Mathf.FloorToInt(_buyValue / 3);
		_itemMesh = "Models/Items/Right_knee_piece";
		_itemSprite = "Sprites/Items/Right_knee_piece";
		_itemTexture = "Textures/Items/shoulder_hip_and_knee_plating";
	}
}
